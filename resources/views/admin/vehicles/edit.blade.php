@php
    $photoInputs = collect(old('photos', $photoUrls ?? []))->pad(3, '');
    $selectedPrimary = old('main_photo_index', $primaryIndex ?? 0);
@endphp
...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Editar veículo</h1>
                <p class="text-sm text-gray-500">Atualize as informações e fotos exibidas no catálogo.</p>
            </div>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ route('admin.vehicles.update', $vehicle) }}" class="space-y-8 rounded-3xl bg-white p-8 shadow ring-1 ring-gray-200">
                @csrf
                @method('PUT')
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select id="color_id" name="color_id" class="mt-1 block w-full rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                            @foreach ($colors as $color)
                                <option value="{{ $color->id }}" @selected(old('color_id', $vehicle->color_id) == $color->id)>
                                    {{ $color->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="year" name="year" type="number" min="1960" max="{{ now()->year + 1 }}" class="mt-1 block w-full" value="{{ old('year', $vehicle->year) }}" required />
                    </div>

                    <div>
                        <x-input-label for="mileage" value="Quilometragem (km)" />
                        <x-text-input id="mileage" name="mileage" type="number" min="0" step="1" class="mt-1 block w-full" value="{{ old('mileage', $vehicle->mileage) }}" required />
                    </div>

                    <div>
                        <x-input-label for="price" value="Valor (R$)" />
                        <x-text-input id="price" name="price" type="number" min="0" step="0.01" class="mt-1 block w-full" value="{{ old('price', $vehicle->price) }}" required />
                    </div>
                </div>

                <div>
                    <x-input-label for="description" value="Descrição detalhada" />
                    <textarea id="description" name="description" rows="6" class="mt-1 block w-full rounded-lg border-gray-300 text-sm leading-relaxed focus:border-indigo-500 focus:ring-indigo-500" required>{{ old('description', $vehicle->description) }}</textarea>
                </div>

                <section>
                    <h2 class="text-lg font-semibold text-gray-900">Fotos do veículo</h2>
                    <p class="mt-1 text-sm text-gray-500">Atualize os links e selecione qual foto ficará em destaque no catálogo.</p>
                            ...
                            <div class="rounded-2xl border border-gray-200 p-4">
                                <div class="flex items-center justify-between">
                                    ...
                                    <label class="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-indigo-600">
                                        <input type="radio" name="main_photo_index" value="{{ $index }}" @checked($selectedPrimary == $index) class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                        Foto principal
                                    </label>
                                </div>
